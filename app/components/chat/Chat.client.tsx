    ...
    <>
      ...
      <ToastContainer
        ...
        icon={({ type }) => {
          ...
          switch (type) {
            case 'success': {
              return <div className="i-ph:check-bold text-easyui-elements-icon-success text-2xl" />;
            }
            case 'error': {
              return <div className="i-ph:warning-circle-bold text-easyui-elements-icon-error text-2xl" />;
            }
          }
...
export const ChatImpl = memo(
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const runAnimation = async () => {
      ...
      // Remove the intro animation that hides elements
      await animate('#examples', { opacity: 0, display: 'none' }, { duration: 0.1 });
      // Remove the animation that affects the logo visibility
...
export function Header() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-easyui-elements-textPrimary cursor-pointer">
        <div className="i-ph:sidebar-simple-duotone text-4xl" />
        <a 
          href="/" 
          className="text-2xl font-semibold text-accent flex items-center"
          onClick={(e) => {
            if (chat.started) {
              if (!window.confirm('Are you sure you want to leave the current chat?')) {
                e.preventDefault();
              }
            }
          }}
        >
          <img src="/logo-dark-styled.png" alt="logo" className="w-[80px] inline-block dark:hidden" />
