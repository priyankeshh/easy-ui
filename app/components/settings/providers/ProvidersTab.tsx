...
export default function ProvidersTab() {
  ...
  const renderProviderCard = (provider: IProviderConfig) => {
    ...
    return (
      <div
        key={provider.name}
        className="flex flex-col provider-item hover:bg-easyui-elements-bg-depth-3 p-4 rounded-lg border border-easyui-elements-borderColor"
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-easyui-elements-textPrimary">{provider.name}</span>
          </div>
          <Switch
            className="ml-auto"
        ...
        {isUrlConfigurable && provider.settings.enabled && (
          <div className="mt-2">
            {envBaseUrl && (
              <label className="block text-xs text-easyui-elements-textSecondary text-green-300 mb-2">
                Set On (.env) : {envBaseUrl}
              </label>
            )}
            <label className="block text-sm text-easyui-elements-textSecondary mb-2">
  ...
  return (
    <div className="p-4">
      ...
      {urlConfigurableProviders.length > 0 && (
        <div className="mt-8">
          <h3 className="text-lg font-semibold mb-2 text-easyui-elements-textPrimary">Experimental Providers</h3>
