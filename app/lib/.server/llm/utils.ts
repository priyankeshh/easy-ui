...
export function simplifyeasyuiActions(input: string): string {
  // Using regex to match easyuiAction tags that have type="file"
  const regex = /(<easyuiAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/easyuiAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<easyuiAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</easyuiAction>`;
    });

  return `<easyuiArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</easyuiArtifact>`;
}
